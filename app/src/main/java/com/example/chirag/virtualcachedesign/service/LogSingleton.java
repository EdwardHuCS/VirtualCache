...
public class LogSingleton {
    ...
    public void send(UserIdPair pair) throws Exception {
        ...
        try {
            ...
            ajax.post(null, "http://198.199.111.14/logs", se, "application/json", new AsyncHttpResponseHandler() {
                @Override
    ...
    public void updateCurrentVideo(String duration, Location location) throws Exception {
        DeviceSchema device;
        if(log.size() > 0){
            device =log.get(log.size()-1);
        } else {
        ...
        if(device.cache.size() > 0){
            ...
            if (location != null) {
                current.location = new Double[]{location.getLongitude(), location.getLatitude()};
            }
